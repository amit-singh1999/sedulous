...
$pdfname = $firstName[0] . "-travel-estimate-" . $dd;
...
curl_setopt_array($curl, array(
    CURLOPT_URL => 'https://sedulous.bitrix24.com/rest/44/mcl8m30hl5f0fblb/disk.folder.addsubfolder',
...
$newfolderid = $response["result"]["ID"];
...
//echo $response;
$response = json_decode($response, true);
$secondfolderid = $response["result"]["ID"];
...
$response = json_decode($response, true);
$thirdfolderid = $response["result"]["ID"];
...
if (isset($_POST['submit'])) {
    ...
    for ($i = 0; $i < $total; $i++) {
        ...
        // $ext = end((explode(".", $name[$i]))); # extra () to prevent notice
    ...
    $sql = "INSERT INTO `customers` ( `email_id`, `travelAdvanceAmount`, `name`, `tripDate`, `employeeNumber`, `site`, `datePrepared`, `reason`, `addComment`, `accountSite`, `accountProject`, `amount`, `employeeSign`, `data`,`userFolderId`,`estimateFolderId`,`expenseFolderId`, `date_time`) VALUES( '$emailID', '$travelAmount', '$fullName', '$trip_date', '$emp_number',  '$site', '$datePrepared', '$reason',  '$addComment', '$accSite', '$accProject' , '$amount', '', '', $newfolderid , $secondfolderid , $thirdfolderid, CURRENT_TIMESTAMP() ) ";
...
<body>
    <div class="container-fluid">
        <form action="PDF.php" method="POST" id="validation">
            ...
            <td style="border: none  !important;">
            <h1 style="color:green"> CONTRACT TRAVEL ESTIMATE</h1><hr>
            </td>
            </tr>
            </table>
...
if (round($sheet_table2))
    $html .= ' <h5>TOTAL OF SHEET 2 =<span> $' . $sheet_table2 . '</span></h5>';
// }
if (round($sheet_table3))
    $html .= ' <h5>TOTAL OF SHEET 3 =<span> $' . $sheet_table3 . '</span></h5>';
$html .= '<h5>TOTAL EXPENSES ESTIMATED = $' . $result . '</h5>
                <h5>Travel Advance Amount Requested (up to 75% total est.) = $' . $travelAmount . '</h5>
...
curl_setopt_array($curl, array(
    ...
    CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
    CURLOPT_CUSTOMREQUEST => 'POST',
    CURLOPT_POSTFIELDS => '  { "id": ' . $secondfolderid . ', 
   "data":{ "NAME": "' . $pdfname . '"
    },
    "fileContent": "' . $output . '" }
